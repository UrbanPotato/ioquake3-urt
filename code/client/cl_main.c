...
void CL_WriteDemoMessage ( msg_t *msg, int headerBytes ) {
        ...
        if (com_newdemoformat->integer) FS_Write(&len, 4, clc.demofile);
}
...
void CL_Record_f( void ) {
  ...
  // sync 0 doesn't prevent recording, so not forcing it off .. everyone does g_sync 1 ; record ; g_sync 0 ..
        ...
        if ( Cmd_Argc() == 2 ) {
                ...
                if(clc.compat) {
                     if (com_newdemoformat->integer) {
                        Com_sprintf(name, sizeof(name), "demos/%s.%s", demoName, NEWDEMOEXT);
                     } else {
                        Com_sprintf(name, sizeof(name), "demos/%s.dm_%d", demoName, com_legacyprotocol->integer);
                     }
                }
        } else {
                int             number;
        ...
        if (!name[0]) {
         Com_Printf ("ERROR: can't find any good filename for demo recording\n");
         return;
        }
...
//        if(!strcmp(DEMOEXT, "urtdemo")) // r00twtfisthis
...
static int CL_WalkDemoExt(char *arg, char *name, int *demofile)
{
        ...
        *demofile = 0;
        ...
        {
                if (com_newdemoformat->integer)
                        Com_sprintf(name, MAX_OSPATH, "demos/%s.%s", arg, NEWDEMOEXT);
                else
                        Com_sprintf(name, MAX_OSPATH, "demos/%s.dm_%d", arg, com_legacyprotocol->integer);
...
static void CL_CompleteDemoName( char *args, int argNum )
{
        if( argNum == 2 )
        {
                ...
                if (com_newdemoformat->integer) {
                   Com_sprintf(demoExt, sizeof(demoExt), ".%s", NEWDEMOEXT);
                } else {
...
void CL_PlayDemo_f( void ) {
        ...
        if (com_newdemoformat->integer) {
          if(ext_test && !Q_stricmpn(ext_test + 1, NEWDEMOEXT, ARRAY_LEN(NEWDEMOEXT) - 1)) {
                Com_sprintf(name, sizeof(name), "demos/%s", arg);
                FS_FOpenFileRead(name, &clc.demofile, qtrue);
                protocol = PROTOCOL_VERSION;
          } else protocol = CL_WalkDemoExt(arg, name, &clc.demofile);
        } else {
          if(ext_test && !Q_stricmpn(ext_test + 1, "dm_", 3))
          {
                protocol = atoi(ext_test + 1 + 3);
...
void CL_Connect_f( void ) {
        ...
        //CL_RequestMotd(); //ioq3-urt commented
        ...
        CL_Disconnect( qtrue );
        //Con_Close(); //workaround for ioq3-urt reconnect workaround
        ...
        //Key_SetCatcher( 0 ); //workaround for ioq3-urt reconnect workaround
        ...
        if (*clc.downloadList) {
                ...
                *s++ = 0;
                ...
                if(!(cl_autodownload->integer & DLF_NO_REDIRECT)) {
                        ...
                        else if (clc.cURLEnabled) { // ioq3-urt: done earlier
                                Com_Printf("WARNING: cURL wasn't initialized\n");
                        }
...
void CL_ConnectionlessPacket( netadr_t from, msg_t *msg ) {
        ...
        if ( !Q_stricmp(c, "getinfo") ) {
                Com_DPrintf ("You or someone else is scanning LAN for servers.\n");
                return;
        }
...
void CL_Frame ( int msec ) {
        ...
        if (clu.TranslationInitialized)
                clu.Com_Translate_DispatchMessages();
...
void CL_StartHunkUsers( qboolean rendererOnly ) {
        ...
        if ( !cls.rendererStarted ) {
                ...
                CL_InitRenderer();
                clu.Com_Totaltimerun_save(); //more hackity
        }
...
void CL_Init( void ) {
        ...
        //ioq3-urt: altered:
        tmpoal = Cvar_Get("s_useopenal", "0", CVAR_ARCHIVE); //notice for cl_voip to work any Cvar_Get is needed since it had CVAR_LATCH
        if (cl_voip->integer) {
                if (Cvar_VariableIntegerValue("rate") < 25000)
                        Com_Printf(S_COLOR_YELLOW "VoIP is enabled but /rate is lower than 25000. A high rate is needed.\n");
                if (!tmpoal->integer)
                        Com_Printf(S_COLOR_YELLOW "VoIP is enabled but /s_useopenal is 0. OpenAL is required.\n");
                if (Cvar_VariableIntegerValue("rate") < 25000 || !tmpoal->integer) {
                        Com_Printf(S_COLOR_YELLOW "Due to that VoIP is now disabled\n");
                        cl_voip->integer = 0;
                }
        }
        //
        ...
        Cmd_AddCommand ("reconnect", clu.CL_Reconnect_f_clu);
...
void CL_Shutdown( char *finalmsg ) {
        ...
        CL_ShutdownAll(); //@r00t Added to close VM nicely on /quit
...
static void CL_SetServerInfo(serverInfo_t *server, const char *info, int ping) {
        if (server) {
                if (info) {
                        ...
                        server->auth = atoi(Info_ValueForKey(info, "auth")); // 4.2
                        server->password = atoi(Info_ValueForKey(info, "password")); // 4.2.2
