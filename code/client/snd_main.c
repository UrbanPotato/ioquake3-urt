                ...
                //  iKALiZER Manager
        {
            ...
            else
            {
                cv = Cvar_Get ( "s_useIkalizer", "0", CVAR_ARCHIVE );
                        ...
                        #ifndef _WIN64  //disable for non-win32 for now
            if (cv->integer && (!started) )
            {
                started = S_SAM_Init( &si );
                                if(started) Com_Printf("* iKALiZER audio Enabled\n");
                Cvar_Set( "s_backend", "IKALIZER" );
            }
                        #endif
        }
                #endif //WIN32 for Ikalizer
                cv = Cvar_Get( "s_useOpenAL", "0", CVAR_ARCHIVE );
                if( cv->integer ) {
                        //OpenAL
                        started = S_AL_Init( &si );
                        if(started) Com_Printf("* OpenAL audio Enabled\n");
