...
void CL_KeyDownEvent( int key, unsigned time )
{
	...
	// ioq3-urt: explicitely minimize in case of alt+tab
	if( clu.cl_alttabminimize->integer && keys[K_ALT].down && key == K_TAB )
	{
...
void Key_SetCatcher( int catcher ) {
	...
	keyCatchers = catcher;
	#ifdef _WIN32
		clu.keyboard_done = qfalse; //ioq3-urt; re-read OS' keyboard repeat/delay settings
	#endif
}
