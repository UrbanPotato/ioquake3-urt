...
void CL_MouseMove(usercmd_t *cmd)
{
        ...
        float zoomsens = clu.CL_Zoom_Sens();
        ...
        if (cl_mouseAccel->value != 0.0f)
        {
                ...
                else
                {
                        ...
                        mx = cl_sensitivity->value * zoomsens * (mx + ((mx < 0) ? -power[0] : power[0]) * cl_mouseAccelOffset->value);
                        my = cl_sensitivity->value * zoomsens * (my + ((my < 0) ? -power[1] : power[1]) * cl_mouseAccelOffset->value);
        ...
        {
                mx *= cl_sensitivity->value * zoomsens;
                my *= cl_sensitivity->value * zoomsens;
        }
...
qboolean CL_ReadyToSendPacket( void ) {
        ...
        if ( clc.netchan.remoteAddress.type == NA_IP) { //ugly ugly hack fix later
                return qtrue;
        }
