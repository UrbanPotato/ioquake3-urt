...
================
*/
LPTOP_LEVEL_EXCEPTION_FILTER prev_except = NULL;
EXCEPTION_POINTERS* ex_info = NULL;
void Sys_SigHandler(int);
long WINAPI MyExceptionFilter(EXCEPTION_POINTERS* info)
{
 ex_info = info;
 Sys_SigHandler(-1);
 ex_info = NULL;
 if (prev_except) return prev_except(info);
...
void Sys_PlatformInit( void )
{
 ...
 prev_except = SetUnhandledExceptionFilter(&MyExceptionFilter);
}
...
int Sys_CrashInfo(char *dst, int dstlen)
{
 int len = 0;
 OSVERSIONINFO ver;
 ...
 GetVersionEx(&ver);
 len+=Q_snprintf(dst+len, dstlen-len, "Running on Windows %d.%d.%d (%s)\n",
  ver.dwMajorVersion,ver.dwMinorVersion,ver.dwBuildNumber,ver.szCSDVersion);

 if (!ex_info) {
  return len+Q_snprintf( dst+len, dstlen-len, "No detailed exception info available\n");
 } else {
  len+=Q_snprintf(dst+len, dstlen-len,
    "Exception %08X at addr %p\n",
    ex_info->ExceptionRecord->ExceptionCode,ex_info->ExceptionRecord->ExceptionAddress);
  if (ex_info->ExceptionRecord->ExceptionCode==EXCEPTION_ACCESS_VIOLATION) {
   len+=Q_snprintf(dst+len, dstlen-len," Can't %s addr %p\n",
    ex_info->ExceptionRecord->ExceptionInformation[0]?"write to":"read from",
    ex_info->ExceptionRecord->ExceptionInformation[1]);
  } else
  if (ex_info->ExceptionRecord->ExceptionCode==EXCEPTION_IN_PAGE_ERROR) {
   len+=Q_snprintf(dst+len, dstlen-len," Can't %s page at addr %p\n",
    ex_info->ExceptionRecord->ExceptionInformation[0]?"write to":"read from",
    ex_info->ExceptionRecord->ExceptionInformation[1]);
  }
  return len;
