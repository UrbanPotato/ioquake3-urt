        ...
        if( !*homePath )
        {
                if( ( p = getenv( "HOME" ) ) != NULL )
                {
                        ...
                        Q_strcat( homePath, sizeof( homePath ),
                                        "/Library/Application Support/UrbanTerror" );
#else
                        Q_strcat( homePath, sizeof( homePath ), "/.urt" );
#endif
                }
        }
...
#define TRACESIZE 100
 ...
 void *buffer[TRACESIZE];
 char **strings;

 nptrs = backtrace(buffer, TRACESIZE);
 len+=Q_snprintf(dst+len, dstlen-len, "backtrace() returned %d addresses:\n", nptrs);

 strings = backtrace_symbols(buffer, nptrs);
 if (strings == NULL) {
  len+=Q_snprintf(dst+len, dstlen-len, " Error: backtrace_symbols==NULL!\n");
  return len;
 }

 for (j = 0; j < nptrs; j++)
  len+=Q_snprintf(dst+len, dstlen-len, " #%2d: %s\n", j,strings[j]);
