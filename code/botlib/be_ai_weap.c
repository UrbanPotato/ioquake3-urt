...
int BotValidWeaponNumber(int weaponnum)
{
        ...
        {
                botimport.Print(PRT_ERROR, "weapon number %d out of range 0..%d\n",weaponnum,weaponconfig->numweapons);
                return qfalse;
        } //end if
        return qtrue;
...
{
        int i, index, bestweapon, numvalid;
        ...
        ws = BotWeaponStateFromHandle(weaponstate);
        if (!ws) {
         botimport.Print(PRT_ERROR, "BotChooseBestFightWeapon: !ws\n");
         return 0;
        }
        wc = weaponconfig;
        if (!weaponconfig) {
         botimport.Print(PRT_ERROR, "BotChooseBestFightWeapon: !weaponconfig\n");
         return 0;
        }

        //if the bot has no weapon weight configuration
        if (!ws->weaponweightconfig) {
         botimport.Print(PRT_ERROR, "BotChooseBestFightWeapon: !ws->weaponweightconfig\n");
         return 0;
        }
...
//                botimport.Print(PRT_ERROR, "BotChooseBestFightWeapon: w%d: weight %f numw %d\n",i,weight,ws->weaponweightconfig->numweights);
        ...
        if (bestweapon==0) {
         botimport.Print(PRT_ERROR, "BotChooseBestFightWeapon: ret %d num %d valid %d\n",bestweapon,wc->numweapons,numvalid);
        }
        return bestweapon;
