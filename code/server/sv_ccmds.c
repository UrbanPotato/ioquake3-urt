...
#include "server.h"
extern cvar_t *com_newdemoformat;
...
static void SVD_StartDemoFile(client_t *client, const char *path)
{
    ...
    if (com_newdemoformat->integer) {
        ...
        v = LittleLong( DEMO_PROTOCOL_VERSION );
        FS_Write ( &v, 4 , file );
...
//ioq3-urt: teleportation; aided by CL_BindTeleport_f()
static void SV_Teleport_f( void ) {
    if (Cmd_Argc() != 5) {
        Com_Printf ("teleport <client num> <x> <y> <z> : teleportation : best use via  /bindteleport <key>\n");
    } else if (!com_sv_running->integer) {
        Com_Printf ("server must be running for teleportation to operate.\n");
    } else {
        sv.gameClients[atoi(Cmd_Argv(1))].origin[0] = atof(Cmd_Argv(2));
        sv.gameClients[atoi(Cmd_Argv(1))].origin[1] = atof(Cmd_Argv(3));
        sv.gameClients[atoi(Cmd_Argv(1))].origin[2] = atof(Cmd_Argv(4));
    }
}
