...
===============
*/
static void SVC_RconRecoveryRemoteCommand( netadr_t from, msg_t *msg ) {
	qboolean	valid;
	unsigned int time;
...
#define SV_OUTPUTBUF_LENGTH (1024 - 16)
	    ...
	    // TTimo - https://zerowing.idsoftware.com/bugzilla/show_bug.cgi?id=534
	    time = Com_Milliseconds();
        if ( sv_rconAllowedSpamIP->string[0] && NET_CompareBaseAdr( from , allowedSpamIPAdress ) || NET_IsLocalAddress(from)) {
         limit = 0;
        }
	
	
	    NET_StringToAdr( sv_rconAllowedSpamIP->string , &allowedSpamIPAdress, NA_IP);
	

        // Prevent using rcon as an amplifier and make dictionary attacks impractical
        if ( limit && SVC_RateLimitAddress( from, 10, 1000 ) ) {
        ...
        if ( ( !sv_rconPassword->string[0] ) ||
                ...
                if ( limit && SVC_RateLimit( &bucket, 10, 1000 ) ) {
                        Com_DPrintf( "SVC_RemoteCommand: rate limit exceeded, dropping request\n" );
                        return;
                }
                valid = qfalse;
			    // MaJ - If the rconpassword is good, allow it much sooner than a bad one.
			    if ( (unsigned)( time - lasttime ) < 180u ) return;
...
void SV_Frame( int msec ) {
        ...
        if (com_speeds2->integer) Time1 = Sys_Microseconds();
        ...
        if (com_speeds2->integer) Time2 = Sys_Microseconds() - Time1;
        ...
        if (com_speeds2->integer) {
         if (!file_speeds2) {
          file_speeds2 = fopen("svframetimes.txt","wt");
          lastTime1 = Time1;
         }
         if (file_speeds2) {
          fprintf(file_speeds2,"%-10d %15I64u %15I64u\n",com_frameNumber,Time1-lastTime1,Time2);
          lastTime1 = Time1;
         }
        } else {
         if (file_speeds2) {
