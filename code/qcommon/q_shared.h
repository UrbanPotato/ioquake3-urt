...
#define CON_VER_SUFFIX "Mitsu/r00t v0.34"
...
#define USE_AUTH
#define NEWDEMOEXT "urtdemo"                       // new demo extension
...
#if 0
#define malloc(x) mallocx(x,__FILE__,__LINE__)
#define free(x) freex(x,__FILE__,__LINE__)
#define calloc(x,y) callocx(x,y,__FILE__,__LINE__)
...
#define MAX_STRING_CHARS1024 1024
...
static ID_INLINE float Q_rsqrt( float number )
{
        ...
        x2 = number * 0.5F;
        t.f  = number;
        t.i  = 0x5f3759df - ( t.i >> 1 );               // what the fuck? //<- lal
        y  = t.f;
...
/*
static ID_INLINE float Q_fabs( float f ) {
        floatint_t fi;
        fi.f = f;
        fi.i &= 0x7FFFFFFF;
        return fi.f;
...
static ID_INLINE vec_t VectorNormalize( vec3_t v ) {
...
static ID_INLINE vec_t VectorNormalize2( const vec3_t v, vec3_t out) {
        float   length, ilength;
...
//int Q_isnan( float x );
#define Q_isnan(x) ((int)((unsigned int)(0x7F800000-((*(unsigned int*)&(x))&0x7FFFFFFF))>>31))
