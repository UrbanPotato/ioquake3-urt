...
Compares without the port //really should do the rate patching here but since it only really works correctly with a modified binary ill leave it and just override the output else-ware
===================
*/
...
qboolean Sys_IsLANAddress( netadr_t adr ) {
        ...
        if( adr.type == NA_IP ) { //spoof NA_type so rate override functions when running binary and client together 
                return qtrue;
        }
...
static qboolean NET_GetCvars( void ) {
        ...
        net_port = Cvar_Get( "net_port", va( "%i", PORT_SERVER ), CVAR_LATCH | CVAR_ARCHIVE );
        modified += net_port->modified;
        net_port->modified = qfalse;

        net_port6 = Cvar_Get( "net_port6", va( "%i", PORT_SERVER ), CVAR_LATCH | CVAR_ARCHIVE );
        modified += net_port6->modified;
        net_port6->modified = qfalse;

        /*      ioq3-urt: dynamic port for NAT clients workaround: If the port is PORT_SERVER both on ipv4 and v6 (the default),
                use a random one from the dynamic range instead (49152-65535). */
        if(!com_dedicated->integer && clu.NET_DynamicPort->integer && net_port->integer == PORT_SERVER && net_port6->integer == PORT_SERVER) {
                int r = 49152 + rand() / (RAND_MAX / (65535 - 49152 + 1) + 1);
                Cvar_Set( "net_port",   va( "%i", r)); modified += net_port->modified; net_port->modified = qfalse;
                Cvar_Set( "net_port6",  va( "%i", r)); modified += net_port6->modified; net_port6->modified = qfalse;
        }
...
void NET_Init( void ) {
        ...
        if(cl_autodownload->integer) {
                if(CL_cURL_Init()) { //ioq3-urt: do it early; + with translation it's required.
                        Com_Printf("* Autodownloading initialized\n");
                } else {
                        Com_Printf(S_COLOR_YELLOW "Autodownloading was not initialized. Appropriate /cl_cURLLib library is needed.\n");
                }
        }

        if(clu.cl_translation->integer) {

                if ( !(cl_autodownload->integer && !clc.cURLEnabled) && CL_cURL_Init()) // if it failed on an enabled autodl, don't even try
                                clu.Com_Translate_Init();

                if (clu.TranslationInitialized)
                        Com_Printf("* Translation initialized; Powered by Google Translate\n");
                else
                        Com_Printf(S_COLOR_YELLOW "Translation support was not initialized. Perhaps an appropriate /cl_cURLLib library is needed.\n");
        }
        #endif
        #endif
}
