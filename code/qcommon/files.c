    ...
    if (fsh[f].zipFile == qtrue) {
        ...
        switch( origin ) {
            case FS_SEEK_SET:
                ...
                Com_Printf (S_COLOR_YELLOW "FS_Seek unzOpenCurrentFile %s\n", fsh[f].name);
...
int FS_ReadFile( const char *qpath, void **buffer ) {
    ...
    qboolean        isConfig;
    int             len;
//  Com_Printf (S_COLOR_YELLOW "FS_ReadFileing %s\n", qpath);
    if ( !fs_searchpaths ) {
        Com_Error( ERR_FATAL, "Filesystem call made without initialization\n" );
    }
...
void FS_Path_f( void ) {
    ...
    for (s = fs_searchpaths; s; s = s->next) {
        if (s->pack) {
            Com_Printf ("[%4i] %s%s\n", s->pack->numfiles,fs_numServerPaks?(FS_PakIsPure(s->pack)?"[PURE] ":"[----] "):"",s->pack->pakFilename);
        } else {
