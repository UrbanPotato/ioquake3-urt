...
static int R_DlightSurface( msurface_t *surf, int dlightBits ) {
    ...
    } else if ( *surf->data == SF_STATIC ) { //@r00tTODO: Is this correct?
        ((srfSurfaceStatic_t *)surf->data)->dlightBits[ tr.smpFrame ] = dlightBits;
    } else {
        dlightBits = 0;
    }

#if 0
    if ( *surf->data == SF_FACE ) { //@r00tdebug
        fprintf(stderr,"FACE\n");
    ...
    } else if ( *surf->data == SF_STATIC ) {
        ((srfSurfaceStatic_t *)surf->data)->dlightBits[ tr.smpFrame ] = dlightBits;
        fprintf(stderr,"STATIC\n");
    } else {
        fprintf(stderr,"WHAT? %d\n",*surf->data); // isGLSLStatic -> SF_STATIC
        dlightBits = 0;
    }
#endif
...
void R_AddWorldSurface( msurface_t *surf, int dlightBits ) {
    ...
    // check for dlighting
    if ( dlightBits ) {
        dlightBits = R_DlightSurface( surf, dlightBits ); //@r00tDebug
