...
typedef struct {
        ...
        image_t                                 *fatlightmap;
        image_t                                 *radbump[4];       //three fat lightmaps for radbumping
...
extern cvar_t   *r_postwidth;
extern cvar_t   *r_postflt;
extern cvar_t   *r_postadd;

extern cvar_t   *r_post_gamma;
extern cvar_t   *r_post_brightness;
extern cvar_t   *r_post_contrast;
extern cvar_t   *r_post_lightness;
extern cvar_t   *r_post_vibrance;
extern cvar_t   *r_post_saturation;

extern cvar_t   *r_bloom;
extern cvar_t   *r_bloomadd;
extern cvar_t   *r_bloomcut;
extern cvar_t   *r_bloomflt;
extern cvar_t   *r_bloomfltwidth;
extern cvar_t   *r_bloomres;
...
*/
//#define GL_Bind(x) GL_BindX(x,__FILE__,__LINE__) //r00tdebug
void    GL_Bind( image_t *image );
//void GL_BindX( image_t *image, char *fn, int ln );
...
static void myGlMultVector(  const float *matrix, vec3_t point )
{
        int i;
        vec3_t eye;
        for ( i = 0 ; i < 3 ; i++ )
...
static void myGlMultVectorRot(  const float *matrix, vec3_t point )
{
        int i;
        vec3_t eye;
        for ( i = 0 ; i < 3 ; i++ )
...
#define GEOMETRY_VERTEX_OFFSET(number, vertex, field) (void *)((number * sizeof(vertex)) + (char*)&(((vertex*)0)->field))
