...
//extern unsigned long long TR_Times[16][2];
#define TIME_START unsigned long long tm_var; Sleep(0); tm_var = Sys_Microseconds()
#define TIME_END(nr) TR_Times[nr][0]+=Sys_Microseconds()-tm_var;TR_Times[nr][1]++

///

unsigned long long TR_Times[7][2] = {{0}};

static FILE *tlf = NULL;

void PrintTimes()
{
 int i;
 if (!tlf) tlf = fopen("times.txt","wt");
 for(i=0;i<sizeof(TR_Times)/sizeof(TR_Times[0]);i++) {
  fprintf(tlf,"%6I64u %5I64u   ",TR_Times[i][0],TR_Times[i][1]);
  TR_Times[i][0]=0;
  TR_Times[i][1]=0;
 }
 fprintf(tlf,"\n");
...
{
	return ((drawSurf_t *)a)->sort - ((drawSurf_t *)b)->sort;
}
/*
===============
R_Radix
...
//TIME_START(SortDrawSurfs);
        ...
        qsort (drawSurfs, numDrawSurfs, sizeof(drawSurf_t), drawSurfCompare ); //@r00t: use qsort
