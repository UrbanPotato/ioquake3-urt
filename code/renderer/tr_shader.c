...
#define FILE_HASH_SIZE          4096 //1024
static  shader_t*               hashTable[FILE_HASH_SIZE];

#define MAX_SHADERTEXT_HASH             4096 //2048
...
static qboolean ParseStage( shaderStage_t *stage, char **text )
{
        ...
        qboolean has_image = qfalse; // ioq3-urt: ensure you have an image before using it (for invalid shaders)
        ...
        if (!has_image) // @r00t: fix for shaders without image, like icy on icyjumps -> make them transparent and not fail
        {
                blendSrcBits = GLS_SRCBLEND_ZERO;
                blendDstBits = GLS_DSTBLEND_ONE;
                if ( !depthMaskExplicit ) depthMaskBits = 0;
                stage->bundle[0].image[0] = tr.whiteImage;
                has_image = qtrue;
                ri.Printf( PRINT_WARNING, "WARNING: Shader '%s' have no image specified. Applying blending to make it invisible.\n", shader.name );
        }
        ...
        // for SRC_ALPHA
        if (has_image && !stage->bundle[0].image[0]->hasAlpha &&
             stage->alphaGen == AGEN_IDENTITY) {
                ...
                ri.Printf( PRINT_WARNING, "WARNING: Shader '%s' has a stage which is missing texture map specification. Dropping stage.\n", shader.name );
                return qfalse;
        }
...
#if 0
        {
         char *tmp = ri.Hunk_Alloc( sizeof( shader_t )+sizeof(int)*2, h_low );
         *(int*)tmp = 0xABCD1234;
         *(int*)(tmp+sizeof(int)+sizeof( shader_t )) = 0xBABE1337;
         newShader = (shader_t*)(tmp+sizeof(int));
        }
#else
...
shader_t *GLSLOptimizeShader(shader_t *checkshader)
{
        ...
        if (checkshader->sort==SS_PORTAL) return checkshader;
        ...
        if (checkshader->optimalStageIteratorFunc==RB_StageIteratorGeneric && checkshader->lightmapIndex<0  && GLSL_CanEmulateDeforms(checkshader) && GLSL_CanEmulateStage(checkshader->stages[0]) )
        {
                ...
                if (checkshader->numUnfoggedPasses==1 || (checkshader->numUnfoggedPasses==2 && checkshader->whiteLightmap) )
                {
                        ...
                        checkshader->optimalStageIteratorFunc= RB_StageIteratorLightmappedMultitexture_GLSL;
                        if (!checkshader->stages[0]->normalmap || !r_bump->integer )
                        {
                                checkshader->stages[0]->program=&tr.fragshader[FS_VERTEX_LIGHT_CHEAP];
                        }
                        else
...
void GLSLLoadImplicitStages(shader_t* checkshader)
{
        ...
        for (j=0;j<checkshader->numUnfoggedPasses;j++)
        {
                ...
                if ( checkshader->stages[j]->bundle[r].isVideoMap==qfalse && checkshader->stages[j]->bundle[r].isLightmap==qfalse &&  checkshader->stages[j]->bumpmap==NULL  &&  checkshader->stages[j]->normalmap==NULL)
                {
                        ...
                        if (!r_bump->integer) continue; // No bump, don't load anything
...
shader_t *R_FindShader( const char *name, int lightmapIndex, qboolean mipRawImage ) {
        ...
        if ( shaderText ) {
                ...
                /*      No longer used. Kept commented for reference:
                        ioq3-urt temporary workarounds: (they have to be before ParseShader() in the 'if' check)
                                models/mapobjects/sinni/fountainrain: ut4_harbortown
                                textures/iran/sd: ut4_iran3
                                textures/fantm_sfx/chrome_dark: ut4_vacuum_beta1a
                                textures/phelps_test/circuits/shiny_black: ut4_sirkitbored_b6
                */
                if (            /*strcmp(name, "textures/phelps_test/circuits/shiny_black") &&
                                strcmp(name, "textures/fantm_sfx/chrome_dark") &&
                                strcmp(name, "textures/iran/sd") &&
                                strcmp(name, "models/mapobjects/sinni/fountainrain") && */
...
void    R_ShaderList_f (void) {
        ...
        for ( i = 0 ; i < tr.numShaders ; i++ ) {
                if ( ri.Cmd_Argc() > 1 ) {
                        ...
                        shader = tr.shaders[i];
                        if (shader->index != i) ri.Printf( PRINT_ALL, "[BAD INDEX %d] ",shader->index);
                }

                ri.Printf( PRINT_ALL, "[#%04i %p] %i ", i,shader,shader->numUnfoggedPasses );
...
#if 0
                {
                 char *tmp = ((char*)shader)-sizeof(int);
                 ri.Printf (PRINT_ALL,  "[%p %p] ",*(int*)tmp,*(int*)(tmp+sizeof(int)+sizeof( shader_t )));
                }
#endif
...
static void ScanAndLoadShaderFiles( void )
{
        ...
        if ( numShaderFiles > MAX_SHADER_FILES ) {
                ri.Printf( PRINT_WARNING, "WARNING: too many shader files (%d) found, MAX_SHADER_FILES is %d!\n",numShaderFiles,MAX_SHADER_FILES );
                numShaderFiles = MAX_SHADER_FILES;
        }
