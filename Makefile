...
# ARCH independent flags, mostly to disable some warnings
GLOBAL_CFLAGS = -Wno-unused-but-set-variable -Wno-unused-variable -Wunused-function -Wno-parentheses \
	-Wno-unused-function
...
ifeq ($(PLATFORM),darwin)
  ...
  BASE_CFLAGS = -Wall -Wimplicit -Wstrict-prototypes $(GLOBAL_CFLAGS)
  CLIENT_CFLAGS =
  SERVER_CFLAGS =
...
ifeq ($(PLATFORM),mingw32)
  ...
  ifeq ($(ARCH),x86_64)
  OPTIMIZEVM = -O2 -finline-functions -fweb -funit-at-a-time -funroll-loops -fpeel-loops
	ifeq ($(USESSE),1)
	OPTIMIZEVM += -msse -msse2 -mfpmath=sse
	endif
  endif
  ifeq ($(ARCH),x86)
	ifeq ($(ONLYMMX),1)
  ...
  ifeq ($(PROFILEGEN), 1)
	...
	ifeq ($(PROFILEUSE), 1)
	  OPTIMIZEVM += -fprofile-use -Wcoverage-mismatch
	endif
  endif
  ...
  ifeq ($(USE_CURL),1)
	...
	ifeq ($(ARCH),x86_64)
	  ...
	  SERVER_LIBS += $(LIBSDIR)/win64/libcurl.a
	else
	  CLIENT_LIBS += $(LIBSDIR)/win32/libcurl_mmx.a
	  SERVER_LIBS += $(LIBSDIR)/win32/libcurl_mmx.a
	endif
	  else
...
ifeq ($(PLATFORM),freebsd)
  ...
  BASE_CFLAGS = $(shell env MACHINE_ARCH=$(ARCH) make -f /dev/null -VCFLAGS) \
	-Wall -fno-strict-aliasing -Wimplicit \
	-DUSE_ICON -DMAP_ANONYMOUS=MAP_ANON $(GLOBAL_CFLAGS)
  CLIENT_CFLAGS = $(SDL_CFLAGS)
  SERVER_CFLAGS =
  HAVE_VM_COMPILED = true
...
ifeq ($(PLATFORM),netbsd)
  ...
  BASE_CFLAGS = -Wall -fno-strict-aliasing -Wimplicit $(GLOBAL_CFLAGS)
  CLIENT_CFLAGS =
  SERVER_CFLAGS =
...
#############################################################################
  ...
  SHLIBEXT=so
  SHLIBCFLAGS=-fPIC $(GLOBAL_CFLAGS)
  SHLIBLDFLAGS=-shared
...
ifeq ($(ARCH),i386)
  Q3DOBJ += \
	  ...
	  $(B)/ded/matha.o
ifeq ($(USESSE),1)
  Q3DOBJ += $(B)/client/snapvector_sse.o
else
  Q3DOBJ += $(B)/client/snapvectora.o
endif
endif
ifeq ($(ARCH),x86)
  Q3DOBJ += \
...
$(B)/client/%.o: $(IOQ3URTDIR)/%.c
	$(DO_CC)

$(B)/client/%.o: $(JSONDIR)/%.c
	$(DO_CC)
